...
def get_all_filers(office, district):
    ...
    logging.info("About to get_all_filers for office {}, district {}".format(office, district))
    ...
    for row in tabs[largest_index].find_all('tr'):
        ...
        if len(cells) == len(headings):
            ...
            for i, cell in enumerate(cells):
                m = re.search('type="SUBMIT" value="(.*)"/>', str(cells[i].contents[0]))
                if m:
                    row_data['Filer ID'] = m.group(1)
...
def get_reports(row):
    ...
    results = []
    for year in YEARS:
        data = {'filerid_in': row['Filer ID'], 'fyear_in':year, 'contributor_in':'', 'amount_in': '0'}
        resp = post(url, data=data)
        resp.raise_for_status()
        soup = BeautifulSoup(resp.text)
        tabs = soup.find_all('table')
